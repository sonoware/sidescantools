...
In the following the most important CFG parameters which are relevant for  processing with the CLI variant are explained. The config is generated by a pydantic model and comes with a schema file (project_info_schema.json) that can be used for further information.
