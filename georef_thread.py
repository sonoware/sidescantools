...
class Georeferencer(QtCore.QRunnable):
    ...
    #}
    warp_options: dict = {
        "Polynomial 1 (recommended)": "SRC_METHOD=GCP_POLYNOMIAL", 
        "Homography (experimental)": "SRC_METHOD=GCP_HOMOGRAPHY"
    }
    ...
    def __init__(
        ...
        warp_algorithm: str = "SRC_METHOD=GCP_POLYNOMIAL",
    ...
    def run(self):
        ...
        vrt_path = self.output_folder / f"{file_name}_stack.vrt"
        ...
        try:
            ...
            self.mosaic(mosaic_tif_path, vrt_path)
            ...
            if chunk_num < len(ch_split) - 1:
                ...
                if not self.dynamic_chunking:
                    ...
                    for i in range(len(gcp_chunk)):
                        GCP = gdal.GCP(lo[i], la[i], 0, im_x[i], im_y[i])
                        GCP_list.append(GCP)

                    gdal_TrsOptions = gdal.TranslateOptions(
                        GCPs = GCP_list, 
                        format = 'GTiff'
                        )

                    gdal.Translate(
                        str(chunk_path), str(im_path), 
                        options=gdal_TrsOptions
                    )

                    if self.active_utm:
                        gdal_wrpOptions = gdal.WarpOptions(
                            resampleAlg = self.resampling_method,
                            transformerOptions = [self.warp_algorithm],
                            polynomialOrder = '1',
                            creationOptions = 'COMPRESS=DEFLATE',
                            dstSRS = self.epsg_code,
                        )
                    else:
                        gdal_wrpOptions = gdal.WarpOptions(
                            resampleAlg = self.resampling_method,
                    ...
                    gdal.Warp(str(warp_path), str(chunk_path), options = gdal_wrpOptions)


            elif chunk_num == len(ch_split) - 1:
                pass

    def mosaic(self, mosaic_tiff, vrt):
        """
        ...
        gdal_vrtOptions = gdal.BuildVRTOptions(
            srcNodata = '0',
            resolution = self.resolution_mode,
            creationOptions = ["COMPRESS=DEFLATE", "TILED=YES"]
        )
        gdal.BuildVRT(str(vrt), TIF, options = gdal_vrtOptions)
        gdal.Translate(str(mosaic_tiff), str(vrt))
